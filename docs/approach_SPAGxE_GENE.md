...
- Applying the **retrospective SPA framework** to ensure accurate p‑values even under extreme phenotypic distributions (e.g., highly unbalanced case‑control ratios).  
...
2. **Retrospective Analysis Framework**  
   The same retrospective likelihood that underpins SPAGxE is used to derive the joint score statistic for the set. Conditioning on phenotype, environment, and covariates avoids fitting a full model per variant and naturally accommodates binary, time‑to‑event, and quantitative traits.
...
- **SPAGxE‑GENE (this work):**  
  **Ma, Y.** (first author), *SPAGxE‑GENE: extending the retrospective SPA G×E framework to gene‑based set tests for rare‑variant gene‑environment interaction analysis*.  
...
All code, documentation, and materials related to **SPAGxE**, **WtGxE**, and **SPAGxE‑GENE** are **Copyright © 2025 Yuzhuo Ma and collaborators**. All rights reserved.  
...
If you wish to use any of these methods in your research, collaborate, or extend them, please contact:  
